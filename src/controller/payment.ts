    ...
    try {
      ...
      if (payment.status != PaymentStatus.OPEN) {
        ...
        if (payment.amount * 100 != paymentAmount) {
          throw new BadRequestError("Cannot confirm payment: amount not correct.");
        }
        payment.status = PaymentStatus.CONFIRMED;
        payment.save();
        await Promise.all(payment.orders.map(order => {
...
const FAILURE_RESPONSE_STRING = "<xml><return_code><![CDATA[FAIL]]></return_code><return_msg><![CDATA[OK]]></return_msg></xml>";
const SUCCESS_RESPONSE_STRING = "<xml><return_code><![CDATA[SUCCESS]]></return_code><return_msg><![CDATA[OK]]></return_msg></xml>";
