...
const ALI_APP_PUBLIC_KEY =
  "";
...
export class AliApiController {
  @HandleError("getVerifyParams")
  static async getVerifyParams(req: Request, res: Response): Promise<void> {
    ...
    let certifyId = "";
    // certifyId = getUserCertifyId(userId)
    if (certifyId) {
    ...
    const verifyParams = {
      ...
      merchant_config: {
        return_url: "www.integ.lt.pbrick.cn/ali/return",
      },
    };
  ...
  static async queryVerify(req: Request, res: Response): Promise<void> {
    ...
    let certifyId = "";
    // 从用户表获取用户certifyId
    // certifyId = getCertifyIdByUser(userId);
    const alipaySdk = new AlipaySdk({
      appId: ALI_APP_ID,
      privateKey: ALI_APP_PRIVATE_KEY,
      encryptKey: ALI_AES_ENC,
